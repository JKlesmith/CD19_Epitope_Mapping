...
"""
Function: Plot scatter of fm vs. fm
"""
# set the tolerance
z_tol = 2
read_tol = 12
...
"""
Function: Identify sites with these off-axis mutations
"""
...
for loc in np_loc:
    ...
    nm = df_site[dset + '_20nm_Epitope_Drop_sd_from_wt']
    ...
    if pm_od_count > 0 or nm_od_count> 0:
        list_any.append(loc)
...
# defines
epi_resi = [140, 144, 202, 203] # FMC
#epi_resi = [140, 144, 146, 200, 202] # 4G7
...
for loc in list_any:
    ...
    # return the distance of the residue
    list_dist.append(np_dist_min[loc - 1])
    
np_wins = np.array(list_dist)  

"""
normalize to total number of residues in distance bins
"""
# create 4 angstrom bins
bins = np.arange(0, 52, 4)

# get the bin number for all residues
digitized = np.digitize(np_dist_min, bins)

# count the number of residues within the bins
bin_counts = [np_dist_min[digitized == i].shape[0] for i in range(1, len(bins))]

# get the bin number for each hit
dig_hit = np.digitize(np_wins, bins)

# count the number of residues within the bins
win_counts = [np_wins[dig_hit == i].shape[0] for i in range(1, len(bins))]

# total sites
tot_sites = np.sum(np.array(win_counts))

frac_bin_wins_norm = np.array(win_counts) / tot_sites
frac_bin_wins_oftot = np.array(win_counts) / np.array(bin_counts)


# plot
plt.figure(figsize=(6,6))
plt.scatter(bins[:-1], frac_bin_wins_norm, s=3, c='black', alpha=0.5)
plt.show()