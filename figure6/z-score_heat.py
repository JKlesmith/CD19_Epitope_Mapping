...
for mut in mutation_types:
    ...
    # normalize the top
    np_top = (df_mut['FMC63_Epitope_Top_sd_from_wt'].values - 
              df_mut['FMC63_Epitope_Display_sd_from_wt'].values)
    
    # get the drop
    np_drp = df_mut['FMC63_Epitope_Drop_sd_from_wt'].values
    
    # return a mask of read counts
    mask_counts = df_mut['FMC63_Epitope_Drop_ref_counts'] < 12
...
# write our csv
with open(pact_path + '/z-score_topFMC63.csv', 'w') as outfile:
    outfile.write(str_head_loc)
    outfile.write(str_head_wt)
    outfile.write(str_top)
    
# write our csv
with open(pact_path + '/z-score_drpFMC63.csv', 'w') as outfile:
    outfile.write(str_head_loc)
    outfile.write(str_head_wt)
